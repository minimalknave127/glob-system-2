...
import { GlobCard, GlobCardFooter } from "../design/card";
    ...
    return (
        <Fragment>
            <GlobCard name="Nový tender">
                ...
                {activeStep === 0 ? <TenderFirstStep setItems={setItems} items={items} /> : activeStep === 1 ? <TenderSecondStep setSuppliers={setSuppliers} suppliers={suppliers} /> : <TenderThirdStep items={items} suppliers={suppliers} />}
                <GlobCardFooter right>
                    {activeStep !== 0 ? <Button onClick={() => handleStep("back")}>Předchozí</Button> : null}
                    {activeStep !== steps.length - 1 ? <Button onClick={() => handleStep("next")}>Další</Button> : null}
                    {activeStep === steps.length - 1 ? <Button onClick={() => alert("done")}>Dokončit</Button> : null}
                </GlobCardFooter>
            </GlobCard>
        </Fragment>
    )
