...
import { Accordion, AccordionDetails, AccordionSummary, Button, TextField, Typography, InputAdornment } from '@material-ui/core';
...
export const TenderFirstStep = (props) => {
    ...
    return (
        <GlobCardSection name="Zadejte položky">
            ...
            <AddItem open={addItemDialog} onClose={() => setAddItemDialog(false)} onSubmit={handleAddNewItem} itemInfo={addItemInfo} />
            <div style={{ display: 'flex', justifyContent: "space-between" }}>
                {/* eslint-disable-next-line no-undef */}
                <Button onClick={() => setImportOpen(true)} className="my-3"><img src={require("../../../assets/icons/sheets.svg")} alt={"Excel icon"} className={classes.customIcon} />Importovat data z excelu</Button>
                <Button color="primary" variant="contained" onClick={() => setImportOpen(true)} className="my-3"><AddIcon className={classes.customIcon} /> Přidat položku</Button>
            </div>
            <hr />
            <TextField label="Hledat..." type="search" onChange={(e) => setSearchInput(e.target.value)}
                InputProps={{
                    startAdornment: (
            ...
            <div className={classes.root}>
                {items.length < 1 ? <h6 className="ml-3">Žádná data nezadána</h6> : null}
                {currentPosts.map((item, index) => (
                    <Accordion key={index} expanded={expanded === `panel${index}`} onChange={handleChange(`panel${index}`)} TransitionProps={{ unmountOnExit: true }} >
                        ...
                        >
                            ...
                            <Typography className={classes.secondaryHeading}>Množství: {item.amount}</Typography>
                            <Typography className={classes.secondaryHeading}>MJ: {item.mj} </Typography>
                                ...
                                <div className="ml-2">
                                    {item.subItems.map((item, index) => (
                                        <div key={index} className={classes.subItem}>
                                            <Typography className={classes.heading}>{item.name}</Typography>
                                            <Typography className={classes.secondaryHeading}>Množství: {item.amount}</Typography>
                                            <Typography className={classes.secondaryHeading}>Cena: {item.price} Kč</Typography>
            ...
            {items.length > 0 ? <Pagination className="mt-5" style={{ margin: '0 auto', width: 'fit-content' }} count={numOfPages} onChange={(e, val) => setCurrentPage(val)} color="primary" /> : null}
