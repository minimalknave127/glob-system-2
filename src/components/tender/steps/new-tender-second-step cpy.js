...
export const TenderSecondStep = (props) => {
    ...
    useEffect(() => {
        setTimeout(() => {
            setItems([{ id: 0, ico: "08365521", name: "GLOB Alu steel" }, { id: 1, ico: "27653021", name: "GLOB Production" }]);
            setLoading(false);
        }, 1500)
    }, []);
    return (
        <GlobCardSection name="Zadejte dodavatele">
            <div style={{ height: 400, width: '100%' }}>
                <DataGrid loading={loading} rows={items} columns={columns} pageSize={5} checkboxSelection localeText={LocaleText} disableColumnFilter onSelectionChange={(e) => setSuppliers(e.rowIds)} />
            </div>
        </GlobCardSection>
    )
}